...
> 说明：本文为便于调研阅读的中文译本。
> **公式以论文风格图片呈现**（见 `figures/yolov1/`），在 GitHub 文件页与 PR 中均可直接阅读。
...
![置信度定义](figures/yolov1/eq_confidence.png)
...
![公式 (1) 测试时类别相关分数](figures/yolov1/eq1_class_score.png)
...
![公式 (2) Leaky ReLU](figures/yolov1/eq2_leaky_relu.png)
...
![损失权重](figures/yolov1/eq_lambda.png)
...
训练中我们优化如下多部分损失函数，即**公式 (3)**（论文原式排版）：

![公式 (3) 多部分损失函数](figures/yolov1/eq3_loss.png)

为便于理解，也可按五项之和展开：

![公式 (3) 分项展开](figures/yolov1/eq3_loss_split.png)

其中，指示函数 1_i^obj 表示物体是否出现在第 i 个网格单元中；1_ij^obj 表示第 i 个单元中的第 j 个边界框预测器对该预测“负责”。
...
![公式 (1)](figures/yolov1/eq1_class_score.png)

**公式 (2)：Leaky ReLU**

![公式 (2)](figures/yolov1/eq2_leaky_relu.png)

**公式 (3)：总损失（论文原式排版）**

![公式 (3)](figures/yolov1/eq3_loss.png)

**公式 (3)：分项展开**

![公式 (3) 分项](figures/yolov1/eq3_loss_split.png)

**VOC 设定下的输出张量**

![输出张量](figures/yolov1/eq_output_tensor.png)
